...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const preferredAgentModel = Effect.fnUntraced(function* (agent: Agent.Info) {
      if (!agent.model) return undefined
      return yield* provider
        .getModel(agent.model.providerID, agent.model.modelID)
    ...
    const routedModel = Effect.fnUntraced(function* (input: {
      ...
      current?: { agent: string | null; model: { providerID: string; id: string } | null } | undefined
    }) {
      const query = input.parts
        ...
        .filter(Boolean)
        .join("\n")
      const preferred = input.current?.model
        ? {
            providerID: ProviderID.make(input.current.model.providerID),
            modelID: ModelID.make(input.current.model.id),
          }
        : yield* currentModel(input.sessionID)
      return yield* provider
        .routeModel({
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          yield* status.set(sessionID, {
            type: "busy",
            title: execution.objective || "Active session",
            metadata: {
              phase: "loop",
              mode: execution.mode,
