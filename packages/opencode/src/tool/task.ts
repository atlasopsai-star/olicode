  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("TaskTool.execute")(function* (
      ...
      const metadata = {
        ...
        model,
        execution: ctx.extra?.execution,
        ...(ctx.extra?.execution && typeof ctx.extra.execution === "object" && "browser" in ctx.extra.execution && ctx.extra.execution.browser
          ? { browser: ctx.extra.execution.browser }
          : {}),
      ...
      if (
        ctx.extra?.execution &&
        typeof ctx.extra.execution === "object" &&
        "browser" in ctx.extra.execution &&
        ctx.extra.execution.browser &&
        !params.prompt.toLowerCase().includes("browser")
      ) {
        return yield* Effect.fail(
          new Error("Browser-task orchestration requires a browser-focused prompt or agent flow before spawning a subtask."),
        )
      }
