...
function makePrompt(input?: { processor?: "blocking" }) {
  ...
  const registry = ToolRegistry.layer.pipe(
    ...
    Layer.provide(BrowserSession.defaultLayer),
...
it.instance("loop injects OliCode harness guidance for focused browser tasks", () =>
  Effect.gen(function* () {
    ...
    yield* prompt.prompt({
      ...
      agent: "build",
      noReply: true,
      parts: [{ type: "text", text: "Use the browser to test checkout flow and keep the answer short" }],
    })
    yield* llm.text("done")
...
it.instance(
  "falls back to routed/default models when an agent's configured model is unavailable",
  () =>
    Effect.gen(function* () {
      ...
      const result = yield* prompt.loop({ sessionID: session.id })
      expect(result.info.role).toBe("assistant")
      expect(result.parts.some((part) => part.type === "text" && part.text === "fallback-ok")).toBe(true)
    }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* prompt.loop({ sessionID: chat.id }).pipe(Effect.forkChild)
      yield* llm.wait(1)
      const running = yield* status.get(chat.id)
      expect(running.type).toBe("busy")
      if (running.type === "busy") {
        expect(running.metadata?.phase).toBe("loop")
        expect(typeof running.metadata?.objective).toBe("string")
      }
      yield* prompt.cancel(chat.id)
      yield* Fiber.await(fiber)
