...
it.instance(
  "routeModel prefers a stronger model within the current provider for build tasks",
  Effect.gen(function* () {
    const model = yield* Provider.use.routeModel({
      mode: "build",
      query: "Implement a new feature and refactor the flow",
      current: {
        providerID: ProviderID.make("custom-anthropic"),
        modelID: ModelID.make("claude-haiku-4-5"),
      },
    })
    expect(String(model.providerID)).toBe("custom-anthropic")
    ...
    config: {
      provider: {
        "custom-anthropic": {
          ...
          options: { apiKey: "test-key" },
          models: {
            "claude-haiku-4-5": { name: "Claude Haiku", tool_call: true, limit: { context: 200000, output: 4096 } },
            "claude-sonnet-4-6": { name: "Claude Sonnet", tool_call: true, limit: { context: 200000, output: 8192 } },
            "claude-opus-4-7": { name: "Claude Opus", tool_call: true, limit: { context: 200000, output: 8192 } },
          },
        },
      },
    },
...
it.instance(
  "routeModel prefers codex-like models when the request explicitly asks for codex",
  Effect.gen(function* () {
    const model = yield* Provider.use.routeModel({
      mode: "build",
      query: "Use codex to fix the bug and ship the patch",
    })
    expect(String(model.providerID)).toBe("custom-openai")
    expect(String(model.id)).toBe("codex-mini-latest")
  }),
  {
    config: {
      provider: {
        "custom-openai": {
          ...
          options: { apiKey: "test-key" },
          models: {
            "gpt-5-mini": { name: "GPT-5 Mini", tool_call: true, limit: { context: 200000, output: 8192 } },
            "codex-mini-latest": { name: "Codex Mini", tool_call: true, limit: { context: 200000, output: 8192 } },
          },
        },
        "custom-anthropic": {
          name: "Custom Anthropic",
...
it.instance(
  "routeModel prefers long-context research models when available",
  Effect.gen(function* () {
    const model = yield* Provider.use.routeModel({
      mode: "research",
      query: "Research and analyze a large codebase with long context",
    })
    expect(String(model.providerID)).toBe("custom-google")
    expect(String(model.id)).toBe("gemini-2.5-pro")
  }),
  {
    config: {
      provider: {
        "custom-google": {
          ...
          options: { apiKey: "test-key" },
          models: {
            "gemini-2.5-pro": { name: "Gemini 2.5 Pro", tool_call: true, limit: { context: 1000000, output: 8192 } },
          },
        },
        "custom-openai": {
          name: "Custom OpenAI",
